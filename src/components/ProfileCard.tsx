...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const cardRef = useRef<HTMLElement | null>(null); // <section> element
  ...
  const handlePointerLeave = useCallback(
    (event: PointerEvent) => {
      ...
      const wrap = wrapRef.current;
      if (!card || !wrap || !animationHandlers) return;
      const { offsetX, offsetY } = event as unknown as MouseEvent; // offsetX/Y exist on PointerEvent in browsers
  ...
  const handleDeviceOrientation = useCallback(
    (event: DeviceOrientationEvent) => {
      ...
      const wrap = wrapRef.current;
      if (!card || !wrap || !animationHandlers) return;
      const { beta, gamma } = event; // can be null
  ...
  useEffect(() => {
    ...
    card.addEventListener('click', requestDeviceMotion);

    // initial hover-like drift
    const initialX = (wrap.clientWidth ?? 0) / 2;
    const initialY = (wrap.clientHeight ?? 0) / 2;
    ...
    return () => {
      ...
      card.removeEventListener('click', requestDeviceMotion);
  ...
  const cardStyle: CSSVars = useMemo(
    () => ({
